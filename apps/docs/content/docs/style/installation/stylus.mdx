...
4. (Optional) Use Stylus’s **Advanced** pane to disable caching or manual updates when you are experimenting with custom themes.
...
- **Styles not injecting**: Enable “Allow cross-site requests” in Stylus and refresh the tab.
- **Host not matched**: Confirm the wildcard entry covers your domain/port (e.g., `*://coolify.local:*/*`).
- **Theme seems stale**: Reinstall the style from the badge or disable and re-enable Instant Mode.
